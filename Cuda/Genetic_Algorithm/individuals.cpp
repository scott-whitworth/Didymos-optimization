...
void Individual::initialize(cudaConstants& cConstant) {
    ...
    this->startParams.y0 = elements<double>( // calculate the starting position and velocity of the spacecraft from Earth's position and velocity and spacecraft launch angles
        ...
        earth.vr+cos(this->startParams.zeta)*sin(this->startParams.beta)*cConstant.v_escape, 
        earth.vtheta+cos(this->startParams.zeta)*cos(this->startParams.beta)*cConstant.v_escape,
        earth.vz+sin(this->startParams.zeta)*cConstant.v_escape);
}
