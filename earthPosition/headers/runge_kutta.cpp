...
T stepSize, elements<T> & y_new, const T & absTol)
{
    ...
    while(curTime>timeInitial) // iterates in reverse
    {
        ...
        //calculate k values
        rkCalc(curTime, timeFinal, stepSize, y, y_new, error);
...
template <class T> void rkCalc(T & curTime, const T & timeFinal, T stepSize, elements<T> y, elements<T> & y_new, elements<T> & error){
