...
T stepSize, elements<T> & y, const T & absTol)
{
    ...
    while(curTime>timeInitial) // iterates in reverse
    {
        ...
        // The absolute value of step size cannot exceed the total time divided by 2 and cannot be smaller 
        // than the total time divided by 1000
...
template <class T> void rkCalc(T & curTime, const T & timeFinal, T stepSize, elements<T> y, 
elements<T> & v, elements<T> & u){
...
template <class T> T calc_scalingFactor(const elements<T> & previous , const elements<T> & difference, 
const T & absTol, T & stepSize)
{
    ...
    // square root of sum of squares of the error from the 6 elements to determine the scale for the time step 
    // of the next iteration
    normTotError = pow(pow(pmError.r,2) + pow(pmError.theta,2) + pow(pmError.z,2) + pow(pmError.vr,2)
    + pow(pmError.vtheta,2) + pow(pmError.vz,2),(T)1/2);
