...
{
	return (-constG * massSun * y.r / (pow(pow(y.r, 2) + pow(y.z, 2), (T)3.0/2))) + (pow(y.vtheta,2) / y.r) +
...
{
	return (-constG * massSun * y.z / (pow(pow(y.r, 2) + pow(y.z, 2), (T)3.0/2))) + accel*sin(calc_tau(coeff,curTime, timeFinal));
